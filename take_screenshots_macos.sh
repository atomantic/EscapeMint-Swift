...
click_at() {
    local x="$1" y="$2"
    osascript -e "
    tell application \"EscapeMint\" to activate
    tell application \"System Events\"
        tell process \"EscapeMint\"
...
capture_window() {
    local output_path="$1"
    osascript -e 'tell application "EscapeMint" to activate' 2>/dev/null
    sleep 0.5
    local wid
    wid=$(get_window_id)
    if [[ -n "$wid" ]]; then
...
capture_locale() {
    ...
    killall "EscapeMint" 2>/dev/null || true
    sleep 1
