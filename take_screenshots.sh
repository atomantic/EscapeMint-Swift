...
#
# Requires: ScreenshotTests target in EscapeMintUITests
#
...
CONFIG_FILE_TMP="/tmp/escapemint_screenshot_config.json"
...
for device_spec in "${DEVICES[@]}"; do
    ...
    for LANG in "${LANGUAGES[@]}"; do
        ...
        if xcodebuild test-without-building \
            ...
            -destination "platform=iOS Simulator,name=$DEVICE_NAME,OS=$DEVICE_OS" \
            -derivedDataPath "$DERIVED_DATA" \
            -only-testing:"EscapeMintUITests/ScreenshotTests/$TEST_METHOD" \
            CODE_SIGNING_ALLOWED=NO \
            -quiet 2>&1; then
